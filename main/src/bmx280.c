...
{
    // чтение регистра под адресом BMX280_REG_CHPID, в котором хранится chip id
...
esp_err_t bmx280_configure(bmx280_t* bmx280, bmx280_config_t *cfg)
{
    ...
    // стр 25 - https://lcgamboa.github.io/picsimlab_docs/0.8.12/pdf/parts/datasheet/input/BMP280.pdf
...
void bmx280_read_task(void *arg) {
    ...
    while (1) {
        ...
        } while(bmx280_isSampling(bmx280)); // true, если измерения еще выполняются, false - уже выполнились
